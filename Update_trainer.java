...
public class Update_trainer implements MouseListener,ActionListener {
	...
	JTextField c_name_t,c_fathername_t,c_email_t,c_address_t,c_contact_t,c_receipt_no_t,c_payment_plan_t,c_height_t,c_weight_t,c_remark_t,c_trainer_name_t,cutomer_id_t;
	JButton c_submit,c_cancle;
	Choice c_gender_l,customer_id_c,c_member_category_t;
	...
	public Update_trainer() {
				...
				f8.setTitle("Update Trainer");
				f8.setLocationRelativeTo(null);
				

		        

		  		c_welcome=new JLabel("Update Trainer");
				...
				customer_id=new JLabel("Trainer ID");
				...
				try
		        {
		            ...
		            String q3="select Trainer_id from add_trainer";
		            ResultSet rest3=obj3.stm.executeQuery(q3);
		            while(rest3.next())
		            {
		                customer_id_c.add(rest3.getString("Trainer_id"));
		            }
		        }
		        catch(Exception ee)
		        {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource()==c_submit)
		{
			...
			{
				Connectionclass ob=new Connectionclass();
				String q="update add_trainer set Name='"+name+"',Father_name='"+fname+"',Email='"+email+"',"
						+ "Address='"+address+"',Contact_no='"+contact+"',Payment_plan='"+pay+"',Height='"+height+"',Weight='"+weight+"',"
								+ "Remark='"+remark+"',Membership_category='"+member_cat+"'";
